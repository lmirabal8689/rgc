...
def start_detection(input_image, output_directory):
  ...
  # The url that will be passed from the argument list.
  url2 = input_image
  ...
  # Keep track of where we are in the page.
  position = 0
  total = len(links)
  for link in links:
    position+=1
    ...
    if link[-4:] == '.jpg':
      print()
      print("*Opening link %d/%d" % (position, total))
      image_link = url2+link
      #print(link)
      print("%d/%d -> %s" % (position, total, image_link))
      ...
      # Draw the region we are detecting objects in
